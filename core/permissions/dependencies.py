...
def require_any_permissions(*permissions: str) -> Callable[[Annotated[User, Depends(get_current_user)]], User]:
    def dependency(current_user: Annotated[User, Depends(get_current_user)]) -> User:
        granted_permissions = ROLE_PERMISSIONS.get(current_user.role, set())
        if not any(permission in granted_permissions for permission in permissions):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You do not have permission to perform this action",
            )
